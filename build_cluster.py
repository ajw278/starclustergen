...
def add_binaries(rs, ms, bf, xb, vb, q, vs=None):
    ...
    if not vs is None:
        ...
        vs_all = np.append(vs, vs_b, axis=1)
    else:
        vs_all = None
...
def plot_pairs(rstars_phys):
    ...
    if Ndim==2:
        weights = 1./(2.*np.pi*dr)/float(len(dr))
    else:
        weights = 1./(4.*np.pi*dr*dr)/float(len(dr))
...
if __name__=='__main__':
    ...
    rs_all, vs_all, ms_all = add_binaries(rs,  ms, bf, xb/pc2cm, vb/km2cm, q, vs=None)
    plot_pairs(rs_all)
