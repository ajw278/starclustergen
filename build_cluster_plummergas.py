	...
	for ig, gamma in enumerate(gammas):
		...
		for irow, alpha in enumerate(alphas):
			for icol, beta in enumerate(betas):
				...
				dname = 'alpha_%.2lf_beta_%.2lf_gamma_%.2lf'%(alpha, beta, gamma)
				...
				if not os.path.isfile('sim_ics_r.npy') or not os.path.isfile('sim_ics_v.npy') or not os.path.isfile('sim_ics_m.npy'):
					...
					tcross = a*pc2cm/plummer_sigmav(0.0,M*Msol2g, a*pc2cm, G=Gcgs)/Myr2s
					texp = tcross*gamma
					print('Tcross:', tcross, 'Texp:', texp)
					#gparams = define_gasjumps(Mgas, -Mgas/trem, njumps=1, tdelay=1.0, tend=tend, ascale=1.0)
					if texp==0.0:
						gparams = None #np.array([Mgas, agas,0.0, trem])
					else:
					...
					if os.path.isfile('sim_gparams.npy'):
				...
				cp.plot_radii(sim, agas=agas, Mgas=Mgas, axtext=txt, axhmr=ax1, axrif=ax2, axErf=ax3)
